...
namespace AdventOfCode.Days
{
    ...
    public class Day10 : BaseDay
    {
        ...
        private long CalcSyntaxScore(string input)
        {
            ...
            {
                prev = next;
                next = prev.Replace("()", string.Empty).Replace("[]", string.Empty).Replace("{}", string.Empty).Replace("<>", string.Empty);
            } while (next.Length < prev.Length);

            var corruptChar = next.FirstOrDefault(c => c is ')' or ']' or '}' or '>');
        ...
        private long CalcAutoCompleteScore(string input)
        {
            ...
            while (next.Length > 0)
            {
                completion += closingChar[next.Last()];
                next = next.ShaveRight(1);
            }
